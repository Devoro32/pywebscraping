...
# https://youtu.be/Q_ZLJ77zApY?list=PLRzwgpycm-FiTz9bGQoITAzFEOJkbF6Qp
from requests import Response
import httpx
from selectolax.parser import HTMLParser
import time
from urllib.parse import urljoin
from dataclasses import asdict, dataclass, fields  #!new
...
def main():
    ...
    for x in range(1, 30):  # up to 10 pages, not including 10
        ...
        #!updated to use append_to_csv
        for url in product_urls:
            print("Url:" + url)
            html = get_html(url)
            append_to_csv(parse_item_page(html))  # getting the title
            time.sleep(0.1)
    ...
    """
        for url in product_urls:
            ...
            time.sleep(1)
    #for product in products: #? Not needed anymore since we are going to export to Json
